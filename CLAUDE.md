...
- **Windows-Laptop für Urlaub einrichten** (Urlaub ~Mitte Juli): `bella_shop` auf GitHub bringen (git init + sichere `.gitignore` wie fliegengitter-web + Plan-Dateien aus `~/.claude/plans/` ins Repo kopieren + push, privat, unter GitHub-User `lafcihsyn`). `fliegengitter-web` ist schon auf GitHub (`lafcihsyn/fliegengitterApp_Shop`). Auf Windows: Node LTS + Git + VS Code + Claude Code + `npm i -g firebase-tools` + `firebase login` + beide Repos per HTTPS klonen + `npm install` in bella_shop/functions. **Vor Abreise: letzten Stand BEIDER Projekte pushen.** Kontinuität läuft über CLAUDE.md (lädt in jedem neuen Chat). Keine Geheimnisse im Code (Stripe/Resend in Firebase) → GitHub sicher.
