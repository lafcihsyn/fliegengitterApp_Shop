...
**Aktueller Stand: v1.20.19** (Stand: 2026-09-20)
...
- **🏖️ Windows-Laptop / Urlaub — AKTIV ab 2026-07-11**: Ab heute (2026-07-11) arbeitet der User vom **Windows-Laptop** (Urlaub Wien **15.07.–15.08.2026**). **Mac-Stand beim Wechsel: alles committet + gepusht, Stand v1.20.15.** Setup ✅ verifiziert (2026-07-02): beide Repos privat auf GitHub (`lafcihsyn/bella_shop` + `lafcihsyn/fliegengitterApp_Shop`, keine Secrets), auf Windows geklont, `firebase login` erledigt, Test-Deploy beider Projekte von Windows erfolgreich. **⚠️ Arbeitsweise mit 2 Rechnern (WICHTIG):** (1) Rechner wechseln → ZUERST `git pull`. (2) Nach Änderungen IMMER `git add`+`commit`+`push` (nicht nur deployen), sonst geht der Stand beim nächsten Wechsel verloren. (3) Beim Rückkehr auf den Mac: `git pull` vor Weiterarbeit. Optional offen: `fliegengitterApp_Shop` auf Private stellen.
...
- **v1.20.19** (2026-09-20) — **Bodenprofil-Warnung in der Schnittliste**: Bestellungen mit **Bodenprofil** wurden übersehen → das normale Rahmenprofil 2× geschnitten (statt oben Rahmen / unten Bodenprofil), auffällig erst beim Zusammenbau. Jetzt markiert die Schnittliste **nur bei aktivem Bodenprofil** genau zwei Zeilen rot: die **Rahmen-Breite** (mit aktivem Bodenprofil-Override, d.h. die Zeile mit ⚡) + die **Bodenprofil**-Zeile. Optik: **roter Rahmen** um die Zeilen + **sanft gleitende Farbe** (`@keyframes bpShimmer`, hellroter Gradient, 3s, kein hartes Blinken) + die **Stück-Zahl** (4. Spalte, z.B. „1") **größer (18px) & fett** in Rot — die entscheidende Anzahl. Respektiert **`prefers-reduced-motion`** (dann statischer roter Rahmen + hellroter Hintergrund, kein Schimmer). Umsetzung rein in [index.html](fliegengitter-web/public/index.html): CSS `.bp-warn` im Inline-`<style>` + `renderTable` (`hasBodenprofil` aus `measureVariants.schwellenlos==='ja' || bodenprofil==='ja'`; `isBodenprofilMat` via `/bodenprofil/i` am Materialnamen; Klasse `bp-warn` an der Cut-Zeile wenn `isBodenprofilMat || (cut.basis==='breite' && hasActiveOverride)`). Keine Daten-/Rechenänderung — reine Anzeige. Anlass: wiederholte Schnittfehler. Live: nur App-Hosting. (User testet mit Mitarbeitern am 2026-09-21.)
- **2026-09-08** — **Webshop Konfigurator Step 2: Profilbild kleiner + Bild & Preis fixiert**: Auf der Maße-&-Farbe-Seite war das Profilbild unnötig groß (füllte die ganze linke Spalte). Jetzt sitzt das **klein skalierte** Profilbild als eigene Karte in der **rechten Sticky-Spalte** über der Preis-Karte „Ihre Konfiguration" — **beide bleiben beim Scrollen stehen**, die Auswahl (Maße/Profilfarbe/Ausführung) scrollt links. Umbau in [views.js](bella_shop/public/js/views.js) `renderStep2`: `galleryHtml` in die `.config-summary`-Sidebar verschoben; **wichtig:** `id="step2-summary"` von der `.config-summary`-div **getrennt** und als inneres div nur um die Preis-Karte gelegt — sonst hätte `updateStep2Summary` (rendert die Preis-Karte bei jeder Eingabe neu) das Bild weggelöscht. Bild verkleinert in [style.css](bella_shop/public/css/style.css) `.model-gallery .gallery-main` (`aspect-ratio:4/3` → feste `height:150px`, `object-fit:contain`; am Test-Channel justiert). Sticky (`.config-summary { position:sticky; top:100px }`) war schon vorhanden. Mobil (≤800px) stapelt wie bisher. Live: nur `hosting:shop`.
- **v1.20.18** (2026-09-08) — **Maß-Bemerkung-Bugfix (nicht gespeichert) + im Bestell-PDF**: Beim Anlegen einer Bestellung ging die **Maß-Bemerkung** („Bemerkung (Maß)") verloren — **beide** Neu-Speichern-Wege ([08-order.js](fliegengitter-web/public/js/08-order.js): `saveAsDraft` ~Z.1333 + Haupt-„Bestellung speichern" ~Z.2676) überschrieben `measureObj.bemerkung` mit `''` statt `m.bemerkung`. Die **Bestell-Bemerkung** (`o.bemerkung`) war nie betroffen; „Bearbeiten"-Speichern (`doSaveOrderEdit` Z.886) übernahm sie korrekt → daher „Nachspeichern geht". Fix: beide Stellen setzen jetzt `m.bemerkung || ''` (Reparatur-Pfad Z.1668 bewusst unangetastet — anderer Kontext). Zusätzlich: die **Maß-Bemerkung wird jetzt im Bestell-PDF** ([05-output.js](fliegengitter-web/public/js/05-output.js) `exportOrderPDF`) **pro Position** angezeigt (gelb hervorgehoben, „Bemerkung: …"), zusätzlich zur bestehenden Bestell-Bemerkung. Etikett (TSPL) unverändert. Anlass: User-Report „Bemerkungen werden nicht gespeichert, Nachspeichern geht, aber nicht gedruckt". Live: nur App-Hosting.
- **v1.20.17** (2026-08-25) — **„Farbe" → „Profilfarbe" (Klarheit ggü. Netz-/Stofffarbe)**: Seit es „Fliegengitter-Netzfarbe"/„Sonnenschutz-Stofffarbe" gibt, war die Profil-/Rahmenfarbe „Farbe" verwechselbar. Umbenannt zu **„Profilfarbe"** — kundenseitig im Webshop ([views.js](bella_shop/public/js/views.js) Feld-Label + Zusammenfassung, [app.js](bella_shop/public/js/app.js) Validierung) + PDF-Rechnung ([pdf.js](bella_shop/functions/src/lib/pdf.js) „Farbe:"→„Profilfarbe:") **und** intern in der BestellApp-Bestellmaske ([08-order.js](fliegengitter-web/public/js/08-order.js): Neu-Formular „Profil-Farbe"→„Profilfarbe", Edit-Formular „Farbe"→„Profilfarbe"). **Bewusst „Farbe" gelassen:** Material-Farbe im Lager + B-Ware (anderer Kontext) + generischer Stepper „Maße & Farbe". E-Mail zeigt die Farbe nur als Wert (kein Label) → unverändert. Live: `hosting:shop` + Function `onPaymentReceived` + BestellApp-Hosting.
- **v1.20.16** (2026-08-25) — **Netzfarben pro Profil einschränkbar + Webshop-Profilkarten größer**: (1) **Netzfarben pro Profil**: neues Stammdaten-Feld **`netzColors`** pro Modell (Block „Erlaubte Netzfarben (Fliegengitter)"), analog zur bestehenden Profil-Farbauswahl (`colors`). Owner hakt pro Profil an, welche Fliegengitter-Netzfarben verfügbar sind — **leer = alle erlaubt** (rückwärtskompatibel). Respektiert überall: BestellApp-Stammdaten-Editor ([06-stammdaten.js](fliegengitter-web/public/js/06-stammdaten.js) `openModelForm` Checkbox-Block `model-netzcolor-cb` + `saveModel` liest `.model-netzcolor-cb:checked` → Feld `netzColors`), BestellApp-Bestellmaske ([08-order.js](fliegengitter-web/public/js/08-order.js) Neu- + Edit-Formular filtern die Netzfarben-Chips nach `currentModel.netzColors`, `withSavedItem` erhält Alt-Werte), Webshop-API ([api.js](bella_shop/functions/src/api.js) gibt `availableNetzColors: m.netzColors||[]` mit) + Webshop-Konfigurator ([views.js](bella_shop/public/js/views.js) filtert die Netzfarb-Folgeauswahl nach `m.availableNetzColors`). Anlass: Slim = nur Schwarz, Pro = Schwarz + Grau (live verifiziert: API liefert `["schwarz"]` bzw. `["schwarz","grau"]`). (2) **Webshop-Profilkarten** (Step 1 „Welches Profil möchten Sie?"): die 2 Profil-Karten füllen jetzt die volle Container-Breite (`.model-grid` `auto-fill`→`auto-fit`) + Profil-Bilder größer (`.model-thumb` 80→120px, `.model-icon` 56→72px), da die Seite nur noch der Profil-Auswahl dient. Live: BestellApp-Hosting (v1.20.16) + `hosting:shop` + Function `api`.
- **2026-08-25** — **Webshop-Konfigurator: kundenfreundliche Begriffe + Kombi-Türart-Logik**: Öffentlicher Konfigurator (fliegengitterwien.at) auf Kunden-Begriffe umgestellt — **„Modell" → „Profil"** (nur Anzeige; die 2 Einträge sind eigentlich Profilarten 18×29 / 26×39), Feld „Netz/Plissee" → **„Ausführung"**, Options-Namen jetzt **aus den Stammdaten** (Firestore: „Fliegengitter"/„Sonnenschutz"/„Fliegengitter/Sonnenschutz Kombi" — der frühere Code-Override in [views.js](bella_shop/public/js/views.js) `VARIANT_OPTION_LABEL_OVERRIDES` wurde **entfernt**, Stammdaten = einzige Quelle). Folge-Farben eindeutig benannt: **„Fliegengitter-Netzfarbe" / „Sonnenschutz-Stofffarbe"** (vorher „Netz-/Plissee-Farbe" — verwechselbar mit der Profil-„Farbe"). **Kombi erzwingt Doppeltür** per ID (`netz_plissee==='kombi'`, wie BestellApp v1.20.14 — NICHT über `nurDoppeltuer`-Flag, das würde in der BestellApp die Kombi-Option bei Einzeltür ausblenden; Einzeltür wird gesperrt) — **und umgekehrt**: Wechsel Kombi→Fliegengitter/Sonnenschutz hebt den Doppeltür-Zwang wieder auf (zurück auf Einzeltür), in [app.js](bella_shop/public/js/app.js) `measureSetVariant`. Kundenseitig **überall konsistent**: Konfigurator + Bestätigungs-Mail **und** Mitarbeiter-Mail ([templates.js](bella_shop/functions/src/email/templates.js) — eine gemeinsame `VARIANT_LABELS`-Liste, `forCustomer`-Schalter wieder entfernt) + PDF-Rechnung ([pdf.js](bella_shop/functions/src/lib/pdf.js)). Zusätzlich Datenschutz-Text „Modell"→„Profil" und 2 (vom Frontend abgefangene) API-Fehlermeldungen umbenannt. **Unverändert:** DB-Werte `netz/plisee/kombi`, Preise, interne BestellApp-Logik; `nurDoppeltuer`-Häkchen bewusst NICHT gesetzt. **Profil-Kurzbeschreibung** („…oder Plissee wählbar") bleibt Stammdaten-Text → passt der User selbst an. Live: `hosting:shop` + Functions `api`, `onOrderCreated`, `onPaymentReceived`. **Offen (nächster Schritt):** Netzfarben pro Profil einschränkbar machen (Slim nur Schwarz, Pro Schwarz+Grau) — braucht neues Stammdaten-Feld „erlaubte Netzfarben pro Profil" (BestellApp + Webshop).
- **2026-08-17** — **Cloud Functions auf Node.js 22 migriert**: Node.js 20 wird am **30.10.2026 abgeschaltet** (danach keine Function-Deploys mehr möglich). Umgestellt: `functions/package.json` `engines.node` „20"→„22" **und** [firebase.json](bella_shop/firebase.json) `functions.runtime` „nodejs20"→„nodejs22". ⚠️ **Wichtig:** `firebase.json` `runtime` hat **Vorrang** vor `engines.node` — nur `engines` zu ändern reichte NICHT (Deploy blieb auf 20). Keine Library-Breaking-Changes nötig: `firebase-functions` ist bereits v6, `firebase-admin` v12 (beide Node-22-fähig). Vorgehen: Canary (nur `onPaymentReceived`) → verifiziert → dann alle 7 Functions (api, stripeWebhook, onOrderCreated, onOrderStatusChange, onPaymentReceived, scheduledBackup, onOrderPlaced). Alle auf nodejs22, `api`-Health-Check grün. Live (alle Functions).
- **2026-08-17** — **PDF-Rechnung: Mehrseiten-Fix**: Rechnungen mit vielen Positionen (> ~11) zerfielen ab Seite 2 — [functions/src/lib/pdf.js](bella_shop/functions/src/lib/pdf.js) zeichnete alles mit **festen Y-Koordinaten ohne Seitenumbruch-Logik** → PDFKit legte bei jedem `doc.text()` jenseits des Seitenrands eine eigene Seite an, sodass jede Tabellenzelle (Bezeichnung/Farbe/Stück/Preis/Gesamt) auf einer **separaten Seite** landete (Bsp. FGO2026-00014: 13 Pos. → 34 kaputte Seiten). Fix in `drawPdf`: echte Seitenumbruch-Logik — Positionen brechen sauber um + wiederholen den Tabellenkopf (`drawTableHeader`); der Summen-/Bank-Block wandert komplett auf eine neue Seite, wenn < 230pt Platz bleiben; Pflicht-Fußzeile jetzt auf **jeder** Seite (`bufferPages:true` + `bufferedPageRange`/`switchToPage`). Lokal verifiziert: 3→1, 13→2, 20→2, 40→4 Seiten, Zeilen intakt. Live: nur Function `onPaymentReceived` (erzeugt die Rechnungs-PDF).
- **v1.20.15** (2026-07-10) — **Modell-Wechsel erhält Varianten (Bugfix, Anlass #2026-00637)**: Beim Ändern des Modells einer bestehenden Bestellung wurden **alle Varianten des Maßes platt zurückgesetzt** (`variants = {}` + Defaults des neuen Modells) → eine **Kombi**-Bestellung wurde beim Wechsel Slim→Pro stumm zu **Netz**, **Plissee-Farbe** ging verloren, und der Preis fiel vom Kombi-Preis (84) auf den Netz-Standard (59). Fix in [08-order.js](fliegengitter-web/public/js/08-order.js) `updateEditMeasureModel` (~Z.506): Werte, die das **neue Modell auch kennt**, bleiben erhalten (Netz/Plissee inkl. Kombi, Türart); nur echte **neue** Varianten (z.B. „Schwellenlos" bei Pro) bekommen ihren Default. Folge-Farben `netzFarbe`/`plisseeFarbe` (hängen an netz_plissee, stehen nicht in `variantIds`) werden separat mitgenommen. Zusätzlich Preis-Vorschlag jetzt **matrix-bewusst** via `computeSuggestedPrice(newModel, doppeltuer, np)` statt `defaultSqmPrice…` → Kombi-Preis bleibt beim Wechsel korrekt. Passt zum Prinzip „kein Auto-Clear" (v1.19.58). #637 muss noch von Hand korrigiert werden (war vor dem Fix schon auf Netz). Live: nur App-Hosting.
- **v1.20.14** (2026-07-10) — **Netz/Plissee-Kombi erzwingt Doppeltür**: Wählt ein Mitarbeiter bei einem Maß **Netz/Plissee = Kombi**, springt die **Türart automatisch auf Doppeltür** (auch aus Einzeltür heraus) und der Türart-Wähler wird **gesperrt** — Kombi + Einzeltür ist damit unmöglich (Netz + Plissee = je eine volle Bahn → 2 Flügel). In [08-order.js](fliegengitter-web/public/js/08-order.js): `updateMeasureVariant` (Neu) + `updateEditMeasureVariant` (Bearbeiten) setzen bei `optionId==='kombi'` → `doppeltuer=true`, `variants.tuerart='doppel'` + Kombi-Preis via `computeSuggestedPrice(mdl,true,'kombi')`; im Render (`renderNewForm` ~Z.1895 + Edit-Loop ~Z.323) wird die Türart-Dropdown bei `netz_plissee==='kombi'` `disabled`. **Bewusst KEIN `nurDoppeltuer`-Häkchen an der Kombi-Option** — das würde Kombi bei Einzeltür verstecken (Mitarbeiter denkt „nicht verfügbar"). So bleibt Kombi immer sichtbar, Klick = Auto-Doppeltür. Webshop machte das schon (views.js). Infotext „(Kombi = Doppeltür)" auf Wunsch wieder entfernt (Sperre bleibt sichtbar durch graues Feld). Live: nur App-Hosting.
- **v1.20.13** (2026-07-10) — **Inegöl-Produktionsstatistik zählt Transport-Status mit**: Die Produktionsliste (welche Bestellungen in Filiale Inegöl produziert wurden) zählte nur Aufträge, die den **Abholbereit**-Status durchliefen. Seit dem Transport-Status (v1.20.6) schieben Inegöl-Aufträge aber zuerst nach **Transport** (Ware noch nicht vor Ort) → sie fehlten in der Statistik. Fix in [index.html](fliegengitter-web/public/index.html): `_producerFilialeId`, `getOrdersProducedByFiliale`, `getMitarbeiter` + der Pro-Mitarbeiter-Zähler bevorzugen jetzt `transportBy`/`transportAt`/`transportByName` vor den `abholbereit`-Feldern und ergänzen „nach Transport verschoben" im Log-Fallback (analog zur schon vorhandenen `computeStats`-Logik). Live: nur App-Hosting.
